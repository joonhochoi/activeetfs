...
Active ETFs는 한국 액티브 ETF의 포트폴리오 구성(PDF, Portfolio Deposit File) 및 변화를 추적하는 Tauri v2 데스크탑 앱이다. 7개 자산운용사에서 보유 종목 데이터를 직접 스크래핑(별도 백엔드 서버 없음)해 로컬 SQLite 데이터베이스에 저장하고, React 대시보드로 비중 변화를 시각화·비교한다. 현재 버전은 **0.4.0**다.
...
- **`components/`**
  ...
  - `Sidebar.tsx` — 상단 탭(List/Search) 구조. **List 탭**: 운용사별 컴팩트 트리(기본 전체 펼침, 즐겨찾기·사용자추가 NEW 배지 포함), 활성 ETF 목록은 `get_etf_enabled_list`, 사용자 추가분은 `get_user_added_etfs`로 가져옴. **Search 탭**: 종목명/코드로 해당 종목을 보유한 모든 ETF 역조회(`search_stock_in_etfs`)
  ...
  - `CompareView.tsx` — 검색 탭에서 선택한 ETF(최대 5개)들의 최신 구성을 메인 뷰에 표 형태로 동시 비교(기준일·구성종목수·상위5/10 합산 비중 + 종목비중 표). 검색으로 찾은 종목은 강조 색으로 표시. `get_latest_holdings` 사용
...
주요 Tauri 커맨드: `get_holdings`, `get_holdings_by_date`, `get_latest_holdings`, `get_latest_date_before`, `check_holdings_exist`, `get_etf_holdings`(fetch.rs), `get_favorite_etfs`, `toggle_etf_favorite`, `get_etf_enabled_list`, `save_etf_enabled_list`, `add_etf_from_url`, `get_user_added_etfs`, `remove_user_etf`, `search_stock_in_etfs`, `export_database`, `import_database`, `get_changelog`, `check_and_update_version`.
