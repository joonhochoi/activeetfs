...
## 5-ter. 추가 직후 차트 제목이 종목코드로 표시되던 버그

> ✅ **처리 완료** — Dashboard가 사용자 추가 ETF 목록(`userEtfs`)을 마운트 시 1회만 불러와, 방금 추가한 ETF는 `resolveEtf`가 못 찾아 제목이 코드로 뜨고 운용사명이 비었다(재시작 후 정상). Dashboard가 `BroadcastChannel('etf-settings')`를 구독해 추가/삭제/가져오기 시 `userEtfs`를 즉시 재조회하도록 수정 → 재시작 없이 정상 표시되고, 추가 직후 Update 버튼도 동작.

---

## 5-quater. 차트 범위 기간 종목 전환 시 유지

> ✅ **처리 완료** — 차트 상단 View Start~End 날짜 입력이 종목을 전환할 때마다 최근 1주로 초기화되던 것을, **최초 1회만 기본값을 설정하고 이후에는 사용자가 지정한 범위를 유지**하도록 변경(`loadHoldings`의 `setViewStartDate/End`를 `prev ?? 기본값` 형태로). 또한 `relevantDates`를 공통 메모로 통합하고, 지정 범위에 해당 종목 데이터가 전혀 없으면(범위 밖) 해당 종목 전체 데이터로 폴백해 빈 차트를 방지. 데이터가 범위보다 짧거나 어긋나도 **차트 제목에는 실제 데이터 기간**이 표기된다(제목이 `relevantDates` 기준).
...
5. ✅ **종목 역조회 + ETF 교차 비교 (구현 완료)** — 사이드바를 List/Search 탭 구조로 개편하고, Search 탭에 "이 종목을 담고 있는 모든 ETF" 역조회를 추가. 백엔드 `search_stock_in_etfs(query)`가 종목명/코드 LIKE 검색으로 ETF×종목별 최신 보유일 행을 반환, 프론트에서 종목별로 묶어 보유 ETF 수·비중과 함께 표시(클릭 시 해당 ETF로 이동). **검색 결과에서 ETF를 최대 5개까지 선택→비교**하면 메인 뷰에 차트 대신 `CompareView`가 떠서 각 ETF의 최신 구성을 표로 동시 비교(기준일·구성종목수·상위5/10 합산 비중 + 전체 종목비중 표). 검색으로 찾은 종목은 강조 색으로 하이라이트(`get_latest_holdings` 사용). (향후 확장: 보유 금액 기준 검색, 공통/차이 종목 요약 등.)
